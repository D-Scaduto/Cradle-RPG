# Cloud Hammer
The Cloud Hammer Path cultivates water and air aura. Cloud Hammer Sacred Artists have a strong command of their aura and bring devastating physical offense and defense to a fight. They a respected, honorable sect favoring physicality over everything else. 


# Type(s) of Aura

Water plus air aura In the correct ratio combine with the Sacred Artists influence creates Cloud Hammer Madra

# Foundation

| Category              | Suggestion                           | Effect                                                                  | Notes                                                         |
| --------------------- | ------------------------------------ | ----------------------------------------------------------------------- | ------------------------------------------------------------- |
| Iron Body             | Stalwart's Armor                     | Reduce all physical damage by 2                                         | Hardens the skin, making it more difficult to cut or puncture |
| Cycling Technique     |                                      | Increased lung performance (survive with less oxygen in the atmosphere) |                                                               |
| Gold Sign             | Cloud Halo floating around your head | None                                                                    | Braodcasts to others the noble Path you're on                 |
| Mundane Attack Damage | Punch                                | +1d10 damage                                                            | You have strong fists                                         |

# Techniques

## [S] Billowing Beam

| Name          | Notes                                                                                                                  |
| ------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Effect        | Force a cylinder of cloud Marda forward, destroying anything it's path for a powerful blunt force. Deals 1d10+5 damage |
| Madra Cost    | 10                                                                                                                     |
| Range         | Melee, Near                                                                                                            |
| Honing Effect | +5 Madra Cost                                                                                                          |

### Honing

| Name                    | Honing Point Cost                               | Notes |
| ----------------------- | ----------------------------------------------- | ----- |
| Intensity/Explosiveness | 2                                               | +1d10 |
| Increase Range          | 4                                               | +Far  |
| Increase Damage         | 2                                               | +5    |
| Reduced Madra Cost      | 1 (doesn't incur the Honing Effect of +5 Madra) | -5    |
| Special Effect          | 4                                               |       |

## [R] Cloaking Haze

| Name          | Notes                               |
| ------------- | ----------------------------------- |
| Effect        | Summon mists/haze to obscure vision |
| Madra Cost    | 15                                  |
| Range         | Near                                |
| Honing Effect | +10 Madra                           |

### Honing

| Name               | Honing Point Cost                               | Notes                                |
| ------------------ | ----------------------------------------------- | ------------------------------------ |
| Increase Range     | 8                                               | ?                                    |
| Increase Damage    | 4                                               | +2 damage per turn while in the mist |
| Reduced Madra Cost | 1 (doesn't incur the Honing Effect of +5 Madra) | -5                                   |

## [F] Thunderhead

| Name          | Notes                                                                                               |
| ------------- | --------------------------------------------------------------------------------------------------- |
| Effect        | Create weapons out of water Madra. It can disolve and reform at will. Use the weapon for 2d10 Damage |
| Madra Cost    | 30                                                                                                  |
| Range         | Melee                                                                                               |
| Duration     | Infinite                                                                                            |
| Honing Effect |  +5 Madra                                                                                           |


### Honing

| Name                    | Honing Point Cost                               | Notes    |
| ----------------------- | ----------------------------------------------- | -------- |
| Intensity/Explosiveness | 2                                               | +1d10    |
| Increase Damage         | 2                                               | +5       |
| Reduced Madra Cost      | 2 (doesn't incur the Honing Effect of +5 Madra) | -3 Madra |

## [F] Kuraudo kusarigama (Cloud Hammer Chain) 

| Name          | Notes                                                                                               |
| ------------- | --------------------------------------------------------------------------------------------------- |
| Effect        | Teather your hammer to you through chains of forged Cloud Madra. Whipping it around you with increased range, speed, and force (+5 Damage) |
| Madra Cost    | 20                                                                                                  |
| Range         | Near (not Melee)                                                                                               |
| Duration      | 3 Rounds                                                                                            |
| Honing Effect |  +5 Madra                                                                                           |


### Honing

| Name                    | Honing Point Cost                               | Notes    |
| ----------------------- | ----------------------------------------------- | -------- |
| Intensity/Explosiveness | 2                                               | +1d10    |
| Increase Damage         | 2                                               | +5       |
| Reduced Madra Cost      | 2 (doesn't incur the Honing Effect of +5 Madra) | -5 Madra |

## [E] Cloud Armor

| Name          | Notes                      |
| ------------- | -------------------------- |
| Effect        | -5 Damage from all attacks |
| Madra Cost    | 15 per turn it's active    |
| Range         | Melee                      |
| Honing Effect | +5                         |

### Honing

| Name                    | Honing Point Cost | Notes                                                                   |
| ----------------------- | ----------------- | ----------------------------------------------------------------------- |
| Intensity/Explosiveness | 1                 | Increase the damage of Unique Feature +5                                |
| Reduced Madra Cost      | 3                 | -5 Madra                                                                |
| Unique Feature          | 4                 | push the Cloud Madra out at any nearby people(5 Madra, Melee, 5 damage) |
